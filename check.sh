...
url=$(curl -s "https://goload.pro" | sed -nE 's_.*<a href="/videos/([^"]*)">_\1_p' | fzf)
[ -z "$url" ] && exit 0 || printf "\033[1;31mSelected $url\n\033[1;36mFetching links"
refr=$(curl -s "https://goload.pro/videos/$url" | sed -nE 's/.*iframe src="(.*)" al.*/\1/p')
...
go_video=$(curl -s -H "X-Requested-With:XMLHttpRequest" "https://goload.pro/encrypt-ajax.php?id=${ajax}&alias=${id}&${token}" | sed -e 's/{"data":"//' -e 's/"}/\n/' -e 's/\\//g' | base64 -d | openssl enc -d -aes256 -K "$second_key" -iv "$iv" | sed -e 's/\].*/\]/' -e 's/\\//g' | grep -Eo 'https:\/\/[-a-zA-Z0-9@:%._\+~#=][a-zA-Z0-9][-a-zA-Z0-9@:%_\+.~#?&\/\/=]*') && [ -z "$go_video" ] && gen_img "gogoplay" "0" || gen_img "gogoplay" "$(printf "$go_video\n" | wc -l)" &

#scraping sbplay link
printf "\n\033[1;34mFetching sbplay2 links"
sb_id=$(printf "$links" | sed -n "s_.*sbplay.*/e/__p" | od -A n -t x1 | tr -d " |\n")
sb_video=$(curl -A "$agent" -s "https://sbplay2.xyz/sources41/616e696d646c616e696d646c7c7c${sb_id}" -H "watchsb:streamsb" | sed -nE 's/.*file":"(.*)","cdn.*/\1/p') && [ -z "$sb_video" ] && gen_img "streamsb" "0" || gen_img "streamsb" "$(printf "$sb_video\n" | wc -l)" &

#xstreamcdn(fembed) links
printf "\n\033[1;34mFetching xstreamcdn links"
fb_id=$(printf "$links" | sed -n "s_.*fembed.*/v/__p")
fb_video=$(curl -s -X POST "https://fembed-hd.com/api/source/$fb_id" -H "x-requested-with:XMLHttpRequest" | sed -e 's/\\//g' -e 's/.*data"://' | tr "}" "\n" | sed -nE 's/.*file":"(.*)","label":"(.*)",.*type.*/\2 > \1/p') && [ -z "$fb_video" ] && gen_img "xstreamcdn" "0" || gen_img "xstreamcdn" "$(printf "$fb_video\n" | wc -l)" &

#doodstream link
printf "\n\033[1;34mFetching doodstream links"
dood_id=$(printf "$links" | sed -n "s_.*dood.*/e/__p")
dood_link=$(curl -A "$agent" -s "https://dood.ws/d/$dood_id" | sed -nE 's/<a href="(.*)" class="btn.*justify.*/\1/p')
sleep .5
dood_video=$(curl -A "$agent" -s "https://dood.ws${dood_link}" | sed -nE "s/.*window.open.*'(.*)',.*/\1/p") && [ -z "$dood_video" ] && gen_img "doodstream" "0" || gen_img "doodstream" "$(printf "$dood_video\n" | wc -l)" &

#mp4upload link
printf "\n\033[1;34mFetching mp4upload links"
mp4up_link=$(printf "$links" | grep "mp4upload")
mp4up_video=$(curl -A "$agent" -s "$mp4up_link" -H "DNT: 1" | sed -nE 's_.*embed\|(.*)\|.*blank.*\|(.*)\|(.*)\|(.*)\|(.*)\|src.*_https://\1.mp4upload.com:\5/d/\4/\3.\2_p') && [ -z "$mp4up_video" ] && gen_img "mp4upload" "0" || gen_img "mp4upload" "$(printf "$mp4up_video\n" | wc -l)" &
wait
