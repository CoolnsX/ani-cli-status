...
url=$(curl -s "$base_url" -A "$agent" | sed -nE 's_.*href="(/v1.*)" title.*_\1_p' | tail -10 | shuf | head -1)
...
al_data=$(curl -s "${base_url}${al}" -A "$agent" | sed -nE 's_.*epslistplace.*>(.*)</div>_\1_p')
al_ep=$(printf "%s" "$al_data" | jq -r '."eptotal"') && al_ep=$((al_ep - 1))
al_links=$(printf "%s" "$al_data" | jq -r ".\"${al_ep}\"[]")

#fetching rush stream links
rush=$(printf "%s" "$tmp" | jq -r '.data[] | select(.type == "RUSH").items[].url' | head -1)
rush_data=$(curl -s "${base_url}${rush}" -A "$agent" | sed -nE 's_.*epslistplace.*>(.*)</div>_\1_p')
rush_ep=$(printf "%s" "$rush_data" | jq -r '."eptotal"') && rush_ep=$((rush_ep - 1))
rush_links=$(printf "%s" "$rush_data" | jq -r ".\"${rush_ep}\"[].vid")

printf "\33[2K\r\033[1;32m link providers (GOGO)>>\033[0m\n$links\n"
...
printf "\033[1;32m link providers (RUSH)>>\033[0m\n%s\n" "$rush_links"
...
printf "\033[1;32m link providers (AL)>>\033[0m\n%s\n" "$al_links"

#odnoklassniki (okru) links
ok_id=$(printf "%s" "$al_links" | sed -nE 's_.*ok.*videoembed/(.*)_\1_p')
printf "\n\033[1;34mFetching okru links < $ok_id"
ok_video=$(curl -s "https://odnoklassniki.ru/videoembed/$ok_id" | sed -nE 's_.*data-options="([^"]*)".*_\1_p' | sed -e 's/&quot;/"/g' -e 's/\u0026/\&/g' -e 's/amp;//g' | tr -d '\\' | sed -nE 's/.*videos":(.*),"metadataE.*/\1/p' | jq -r '.[].url') && [ -z "$ok_video" ] && gen_img "okru" "0" || gen_img "okru" "$(printf "%s\n" "$ok_video" | wc -l)" &

#streamlare
lare_id=$(printf "%s" "$al_links" | sed -nE 's_.*streamlare.*/e/(.*)_\1_p')
printf "\n\033[1;34mFetching okru links < $lare_id"
lare_video=$(curl -s -H "x-requested-with:XMLHttpRequest" -X POST "https://streamlare.com/api/video/download/get" -d "{\"id\":\"$lare_id\"}" -H "x-csrf-token:$(curl -s "https://streamlare.com/e/$lare_id" | sed -nE 's/.*csrf-token.*content="(.*)">/\1/p')" -H "content-type:application/json;charset=UTF-8" | tr -d '\\' | sed -nE 's/.*label":"([^"]*)",.*url":"([^"]*)".*/\1 >\2/p') && [ -z "$lare_video" ] && gen_img "streamlare" "0" || gen_img "streamlare" "$(printf "%s\n" "$lare_video" | wc -l)" &
